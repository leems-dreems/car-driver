...
## The number of RayCast3Ds that this vehicle uses for close avoidance. Must be divisible by 4
var steering_ray_count : int = 16
var steering_ray_length := 8
var max_antenna_length := 16
var min_antenna_length := 8
var current_antenna_length := 16
...
func set_interest_vectors(_target_global_position: Vector3) -> void:
  ...
  current_antenna_length = clampf(speed, min_antenna_length, max_antenna_length)
  for _raycast in antenna_raycasts:
  ...
  if _danger_amount_left > 0.0 or _danger_amount_right > 0.0:
    ...
    interest_vectors[steering_ray_count / 2] *= (_danger_amount_left + _danger_amount_right) * antenna_multiplier
