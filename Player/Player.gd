...
func _physics_process(delta: float) -> void:
  ...
  if is_in_vehicle:
    ...
    if Input.is_action_just_pressed("Toggle Transmission"):
      current_vehicle.automatic_transmission = !current_vehicle.automatic_transmission
    
    if Input.is_action_just_pressed("Shift Up"):
      current_vehicle.manual_shift(1)
    ...
    if aim_collider != null and aim_collider.is_in_group('vehicle_door'):
