...
## Grass spray emitter
@onready var _grass_spray: CarGameParticles = $Spray
    ...
    for wheel: Wheel in vehicle.wheel_array:
      if absf(wheel.slip_vector.x) > lateral_slip_threshold or absf(wheel.slip_vector.y) > longitudinal_slip_threshold:
        ...
        if distance_from_last_decal > skidmark_length:
          ...
          match wheel.last_collision_group:
            "Road":
            ...
            _:
              ...
              if wheel.spin >= 0:
                _spray_vector = wheel.global_transform.basis.z.rotated(wheel.global_transform.basis.x, -PI / 4) * wheel.spin * wheel.tire_radius * 0.5
              else:
                _spray_vector = (-wheel.global_transform.basis.z).rotated(wheel.global_transform.basis.x, -PI / 4) * wheel.spin * wheel.tire_radius * 0.5
          ...
          match wheel.last_collision_group:
            "Road": new_skidmark.albedo_mix = 1
