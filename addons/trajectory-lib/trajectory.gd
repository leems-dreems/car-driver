class_name TrajectoryLib extends Node
...
static func fixed_target(proj_pos : Vector3, proj_speed : float, target_pos : Vector3, gravity : Vector3) -> Array:
	...
	if gravity_dir != Vector3.DOWN:
		...
		proj_pos *= gravity_basis
		target_pos *= gravity_basis
...
## 
## space_state [PhysicsDirectSpaceState3D]: The physics space that will be tested
